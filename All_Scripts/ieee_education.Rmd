...
```{r}
ieee_metaverse = read_xlsx("EDU_V2.xlsx") 
...
d1 %>%
  ...
  mutate(`IEEE Publications on Education` = n) %>%
  ggplot(aes(DATE, `IEEE Publications on Education`)) +
...
edges_x = dgx2 |>
  ...
  filter(weight > 600 | (from %in% c("METAVERSE") & width >= 27 / 2000) | (to %in% c("METAVERSE") & width >= 27 / 2000))
...
edges_edu = edges |>
  filter(from %in% c("EDUCATION"))


visNetwork(nodes, edges_edu, width = "100%", height = "100vh") %>%
...
```{r}
dgx2 |>
  filter(X1 %in% c("METAVERSE")) |>
  arrange(desc(count))
```

```{r}
dgx2 |>
  filter(X2 %in% c("METAVERSE")) |>
  arrange(desc(count))
```
